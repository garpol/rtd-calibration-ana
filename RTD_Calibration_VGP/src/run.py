...
class Run:
    def __init__(self, filename: str, logfile: pd.DataFrame) -> None:
        """
        Inicializa el objeto Run cargando los datos del archivo, filtrando canales defectuosos, asociando los 
        sensores y leyendo la información general del experimento.
        ...
        # Filtrar el archivo y los datos, estos métodos se efectuan al crear el objeto Run de forma automática
    ...
    def load_temperature_file(self) -> None:
        ...
        try:
            ...
            # Primero buscar en la carpeta local del repositorio: RTD_Calibration_VGP/data/temperature_files
            ...
            if local_base.exists():
                # Buscar recursivamente en la carpeta local
                text_file = glob.glob(os.path.join(str(local_base), "**", self.filename + ".txt"), recursive=True)
                if text_file:
                    path_used = local_base

            # Si no se encuentra localmente, volver a intentar en CERNBox (ruta original)
            if not text_file:
                cern_path = "/eos/user/j/jcapotor/RTDdata/"
                text_file = glob.glob(os.path.join(cern_path, "**", self.filename + ".txt"), recursive=True)
                if text_file:
                    path_used = Path(cern_path)

            if not text_file:
                raise FileNotFoundError(f"No se encontró el archivo '{self.filename}' en '{local_base}' ni en CERNBox.")

            self.path_to_file = text_file[0]
            print(f"Archivo de temperatura encontrado: {self.path_to_file} (buscado en: {path_used})")
            ...
            print(f"Valores NaN (contador): {nan_count_after_dropping}")
             ...
             # Reemplazar columnas con más de 40 valores NaN con NaN al completo porque no nos fiamos de las medidas de este sensor
            ...
            # Identificar columnas que tienen al menos un NaN pero menos de 40
    ...
    def create_offset_plot(self, output_dir: str = "./Plots/") -> None:
        ...
        mask = (self.temperature_data.index >= time_20min) & (self.temperature_data.index <= time_40min)
        filtered_data = self.temperature_data.loc[mask]
            ...
            if sensor in filtered_data.columns:
                for other_sensor in sensor_ids:
                    if other_sensor in filtered_data.columns:
                        ...
                        #offset_centered = offset - offset_mean #queremos restar el primer minuto y no la media al final
    ...
    def offsets(self, tini: int = 20, tend: int = 40) -> pd.DataFrame:
        ...
        usando únicamente los últimos 20 min del run (del minuto 20 al 40).

        Parámetros:
            tini (int): Minuto inicial relativo al final del run (por defecto 20).
            tend (int): Minuto final relativo al final del run (por defecto 40).

        Retorna:
            pd.DataFrame: Matriz simétrica de offsets entre sensores.
        """
        try:
            ...
            # Convertir tiempos relativos a timestamps absolutos
            time_end = self.temperature_data.index.max()
            time_start = self.temperature_data.index.min()
           
            time_20min_start = time_start + pd.Timedelta(minutes=tini)
            time_20min_end = time_start + pd.Timedelta(minutes=tend)

            # Filtrar los datos en el rango deseado
            mask = (self.temperature_data.index >= time_20min_start) & (self.temperature_data.index <= time_20min_end)
            filtered_data = self.temperature_data.loc[mask]

            #filtered_data = self.temperature_data.loc[time_20min_start:time_20min_end]

            if filtered_data.empty:
                raise ValueError("No hay datos en los últimos 20 minutos para calcular offsets.")

            # Determinar sensores activos
            active_sensors = [s for s in self.sensor_mapping.values() if s in filtered_data.columns]

            # Inicializar matriz de offsets
            offset_matrix = pd.DataFrame(index=active_sensors, columns=active_sensors, dtype=float)

            # Calcular diferencias promedio entre pares de sensores
            for i, sensor1 in enumerate(active_sensors):
                for j, sensor2 in enumerate(active_sensors):
                    if i == j:
                        offset_matrix.loc[sensor1, sensor2] = 0
                    elif i < j:
                        col1, col2 = filtered_data[sensor1], filtered_data[sensor2]
                        if col1.isna().all() or col2.isna().all():
                            offset_matrix.loc[sensor1, sensor2] = np.nan
                            offset_matrix.loc[sensor2, sensor1] = np.nan
                        else:
                            mean_offset = (col1 - col2).mean()
                            offset_matrix.loc[sensor1, sensor2] = mean_offset
                            offset_matrix.loc[sensor2, sensor1] = -mean_offset

            print("Offsets medios calculados para los últimos 20 minutos del run.")
    ...
    def stat_err_offsets(self, tini: int = -20, tend: int = 0) -> pd.DataFrame:
        """
        Calcula el error RMS respecto al offset medio para cada pareja de sensores,
        usando únicamente los últimos 'tend - tini' minutos del run (por defecto, los últimos 20 minutos).

        Parámetros:
            tini (int): Minuto inicial relativo al final del run (por defecto -20).
            tend (int): Minuto final relativo al final del run (por defecto 0).

        Retorna:
            pd.DataFrame: Matriz simétrica de errores RMS entre sensores.
        """
        try:
            ...
            time_end = self.temperature_data.index.max()
            time_20min_start = time_end + pd.Timedelta(minutes=tini)
            time_20min_end = time_end + pd.Timedelta(minutes=tend)
            ...
            if not active_sensors:
                raise ValueError("No hay sensores activos para calcular errores RMS.")

            # Inicializar matriz RMS
            rms_matrix = pd.DataFrame(index=active_sensors, columns=active_sensors, dtype=float)

            # Calcular error RMS para cada par de sensores
            for i, sensor1 in enumerate(active_sensors):
                for j, sensor2 in enumerate(active_sensors):
                    if i == j:
                        rms_matrix.loc[sensor1, sensor2] = 0
                    elif i < j:
                        col1, col2 = filtered_data[sensor1], filtered_data[sensor2]
                        if col1.isna().all() or col2.isna().all():
                            rms_matrix.loc[sensor1, sensor2] = np.nan
                            rms_matrix.loc[sensor2, sensor1] = np.nan
                        else:
                            mean_offset = self.offsets_data.loc[sensor1, sensor2]
                            rms_error = ((col1 - col2 - mean_offset) ** 2).mean() ** 0.5 #aqui se calcula!
                            rms_matrix.loc[sensor1, sensor2] = rms_error
                            rms_matrix.loc[sensor2, sensor1] = rms_error

            print("Errores RMS calculados para los últimos 20 minutos del run.")
